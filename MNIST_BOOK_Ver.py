    ...
    with tf.Session() as sess:
        ...
        for i in range(TRAINING_STEPS):
            if i % 100 == 0:
                ...
                print("After %d trainning steps, validation accuracy is %g" % (i, validate_acc))
            xs, ys = mnist.train.next_batch(BATCH_SIZE)
            sess.run(train_op, feed_dict={x: xs, y_: ys})
        test_acc = sess.run(accuracy, feed_dict=test_feed)
        print("After %d trainning steps, test accuracy is %g" % (TRAINING_STEPS, test_acc))

def main(argv = None):
    mnist = input_data.read_data_sets("~/desktop/python", one_hot=True)
    train(mnist)
